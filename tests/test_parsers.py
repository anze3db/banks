...
class TestParsers(TestCase):
    def test_bankinter(self):
        res = bankinter_parser.parse(
            os.path.join(sys.path[0], "tests", "files", "bankinter.xls")
        )
    ...
    def test_personal(self):
        res = personal_parser.parse(
            os.path.join(sys.path[0], "tests", "files", "personal.xls")
        )
        self.assertIs(len(res), 3)

    def test_business(self):
        res = business_parser.parse(
            os.path.join(sys.path[0], "tests", "files", "business.csv")
        )
